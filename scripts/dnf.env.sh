# Aditional environment setting for dnf
...
# Set a list of command substitutions needed by dnf
fakechroot_dnf_env_cmd_subst="@MOUNT@=/bin/true
@UMOUNT@=/bin/true
@CHROOT@=${fakechroot_bindir:-@sbindir@}/chroot.fakechroot
@LDCONFIG@=/bin/true
@LDD@=${fakechroot_bindir:-@bindir@}/ldd.fakechroot"

FAKECHROOT_CMD_SUBST="${FAKECHROOT_CMD_SUBST:+$FAKECHROOT_CMD_SUBST:}`echo \"$fakechroot_dnf_env_cmd_subst\" | tr '\012' ':'`"
...
fakechroot_dnf_env_paths=`
    cat /etc/ld.so.conf /etc/ld.so.conf.d/* 2>/dev/null | grep ^/ | while read fakechroot_dnf_env_d; do
        printf '%s:' "$fakechroot_dnf_env_d"
    done
`
if [ -n "$fakechroot_dnf_env_paths" ]; then
    LD_LIBRARY_PATH="${LD_LIBRARY_PATH:+$LD_LIBRARY_PATH:}${fakechroot_dnf_env_paths%:}"
