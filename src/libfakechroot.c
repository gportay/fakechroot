...
void fakechroot_init (void)
{
    ...
    if (!first) {
        ...
        if (exclude_path)
            exclude_init(exclude_path, exclude_symlink_list, exclude_symlink_length, list_symlink_max);
...
LOCAL int fakechroot_localsymlink (const char * p_path)
{
    ...
    return fakechroot_is_excluded (v_path, exclude_symlink_list, exclude_symlink_length, list_symlink_max);
}
